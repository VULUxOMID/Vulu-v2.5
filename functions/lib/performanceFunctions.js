...
exports.cleanupPerformanceData = exports.aggregatePerformanceStats = exports.processPerformanceAlerts = exports.generateStreamQualityReport = void 0;
 ...
 */
exports.generateStreamQualityReport = functions.https.onCall(async (data, context) => {
    var _a;
    try {
        ...
        if (!streamId || !startTime || !endTime) {
            throw new functions.https.HttpsError('invalid-argument', 'streamId, startTime, and endTime are required');
        }
        ...
        if (metricsQuery.empty) {
            throw new functions.https.HttpsError('not-found', 'No performance data found for the specified period');
        }
        ...
        throw new functions.https.HttpsError('internal', 'Failed to generate stream quality report');
    }
});
/**
 * Process performance alerts
 */
exports.processPerformanceAlerts = functions.pubsub
 ...
 */
exports.aggregatePerformanceStats = functions.pubsub
    ...
    try {
        ...
        for (const [streamId, streamMetricsList] of streamMetrics) {
            ...
            batch.set(statsRef, Object.assign(Object.assign({ streamId, period: {
                    start: oneHourAgo,
                    end: admin.firestore.Timestamp.now()
                } }, aggregatedStats), { createdAt: admin.firestore.FieldValue.serverTimestamp() }));
            aggregatedCount++;
        }
 ...
 */
exports.cleanupPerformanceData = functions.pubsub
 ...
 */
async function generateQualityReport(streamId, hostId, metrics, start, end) {
    try {
        ...
        const avgConnectionQuality = metrics.reduce((sum, m) => {
            return sum + (qualityScores[m.connectionQuality] || 0);
        }, 0) / totalMetrics;
        // Detect major issues
 ...
 */
async function analyzePerformancePatterns(metrics) {
    const alerts = [];
 ...
 */
function detectMajorIssues(metrics) {
    const issues = [];
 ...
 */
function generateRecommendations(avgLatency, avgJitter, avgPacketLoss, qualityDistribution) {
...
//# sourceMappingURL=performanceFunctions.js.map