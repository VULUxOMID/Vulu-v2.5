...
exports.onParticipantLeave = exports.onParticipantJoin = exports.processScheduledTasks = exports.cleanupStaleParticipants = void 0;
...
exports.cleanupStaleParticipants = functions.pubsub
    ...
    try {
        ...
        const staleThreshold = new admin.firestore.Timestamp(now.seconds - 120, // 2 minutes ago
        now.nanoseconds);
        ...
        for (const streamDoc of activeStreamsSnapshot.docs) {
            ...
            await db.runTransaction(async (transaction) => {
                ...
                const updatedParticipants = currentStreamData.participants.filter((p) => !staleUserIds.includes(p.id) || p.role === 'host');
            ...
            const remainingParticipants = streamData.participants.filter((p) => p.isActive && !staleParticipantsSnapshot.docs.some(doc => doc.id === p.id));
 ...
 */
async function scheduleStreamAutoEnd(streamId) {
    try {
 ...
 */
async function processAutoEndStream(streamId) {
    try {
        const streamRef = db.collection('streams').doc(streamId);
        ...
        const activeViewers = streamData.participants.filter((p) => p.isActive && p.role !== 'host');
 ...
 */
exports.onParticipantJoin = functions.firestore
 ...
 */
exports.onParticipantLeave = functions.firestore
    .document('streams/{streamId}/participants/{userId}')
    ...
    if (beforeData.isActive && !afterData.isActive) {
        ...
        try {
            ...
            const leaveTime = ((_a = afterData.leftAt) === null || _a === void 0 ? void 0 : _a.toDate()) || new Date();
...
//# sourceMappingURL=participantManagement.js.map