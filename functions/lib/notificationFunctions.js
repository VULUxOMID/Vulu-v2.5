...
exports.cleanupNotificationData = exports.processNotificationReceipts = exports.sendBulkNotifications = exports.sendPushNotification = void 0;
...
exports.sendPushNotification = functions.https.onCall(async (data, context) => {
    try {
        ...
        if (!userId || !type || !title || !body) {
            throw new functions.https.HttpsError('invalid-argument', 'userId, type, title, and body are required');
        }
        ...
        const results = await sendExpoPushNotifications(tokens, {
            title,
            body,
            data: Object.assign(Object.assign({}, notificationData), { notificationId: notificationRef.id, type })
        });
...
exports.sendBulkNotifications = functions.https.onCall(async (data, context) => {
    try {
        ...
        if (!userIds || !Array.isArray(userIds) || !type || !title || !body) {
            throw new functions.https.HttpsError('invalid-argument', 'userIds (array), type, title, and body are required');
        }
        if (userIds.length > 1000) {
            throw new functions.https.HttpsError('invalid-argument', 'Maximum 1000 users per bulk notification');
        }
        const results = [];
        const batchSize = 100; // Process in batches to avoid timeouts
        for (let i = 0; i < userIds.length; i += batchSize) {
            const batch = userIds.slice(i, i + batchSize);
            const batchResults = await Promise.all(batch.map(userId => processBulkNotificationForUser(userId, type, title, body, notificationData)));
 ...
 */
exports.processNotificationReceipts = functions.pubsub
        ...
        for (const doc of pendingQuery.docs) {
            ...
            if (notification.pushResults && notification.pushResults.length > 0) {
                const ticketIds = notification.pushResults
                    .filter((result) => result.status === 'ok' && result.id)
 ...
 */
exports.cleanupNotificationData = functions.pubsub
...
async function getUserPushTokens(userId) {
    try {
        const tokensQuery = await db.collection('notificationTokens')
            ...
            .where('isActive', '==', true)
            .get();
        return tokensQuery.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
    }
    catch (error) {
...
async function sendExpoPushNotifications(tokens, notification) {
    try {
        const messages = tokens
            .filter(tokenData => expo_server_sdk_1.Expo.isExpoPushToken(tokenData.token))
            .map(tokenData => ({
            to: tokenData.token,
            sound: 'default',
        ...
        for (const chunk of chunks) {
            try {
                ...
                tickets.push(...chunk.map(() => ({ status: 'error', message: 'Failed to send' })));
            }
        }
        return tickets;
    }
 ...
 */
async function processBulkNotificationForUser(userId, type, title, body, notificationData) {
    try {
