...
exports.generateModerationReport = exports.cleanupModerationData = exports.processExpiredTimeouts = exports.analyzeChatMessage = void 0;
 ...
 */
exports.analyzeChatMessage = functions.firestore
    ...
    try {
        ...
        const analysisResult = await analyzeMessageContent(messageData.message, messageData.senderId, streamId);
        // If violation detected, take action
        if (analysisResult.isViolation) {
            await handleModerationViolation(streamId, messageId, messageData.senderId, analysisResult);
        }
        // Log analysis result
 ...
 */
exports.processExpiredTimeouts = functions.pubsub
    ...
    try {
        ...
        for (const doc of expiredTimeoutsQuery.docs) {
            ...
            if (streamDoc.exists()) {
                ...
                const updatedParticipants = streamData.participants.map((p) => p.id === action.targetUserId ? Object.assign(Object.assign({}, p), { isMuted: false, isTimedOut: false }) : p);
 ...
 */
exports.cleanupModerationData = functions.pubsub
 ...
 */
exports.generateModerationReport = functions.https.onCall(async (data, context) => {
    var _a;
    try {
        ...
        if (streamData.hostId !== context.auth.uid &&
            !((_a = streamData.moderatorIds) === null || _a === void 0 ? void 0 : _a.includes(context.auth.uid))) {
        ...
        throw new functions.https.HttpsError('internal', 'Failed to generate moderation report');
    }
});
/**
 * Analyze message content for violations
 */
async function analyzeMessageContent(message, userId, streamId) {
    try {
        const violations = [];
        let severity = 0;
        ...
        if ((userStatus === null || userStatus === void 0 ? void 0 : userStatus.warningCount) > 2) {
 ...
 */
async function handleModerationViolation(streamId, messageId, userId, analysisResult) {
    try {
        ...
        if (suggestedAction === 'timeout') {
            ...
            actionData.expiresAt = admin.firestore.Timestamp.fromMillis(Date.now() + actionData.duration * 60 * 1000);
        }
 ...
 */
async function applyAutomatedAction(streamId, userId, action, duration) {
    try {
        ...
        switch (action) {
            case 'mute':
                updatedParticipants = updatedParticipants.map(p => p.id === userId ? Object.assign(Object.assign({}, p), { isMuted: true }) : p);
                break;
            case 'timeout':
                updatedParticipants = updatedParticipants.map(p => p.id === userId ? Object.assign(Object.assign({}, p), { isMuted: true, isTimedOut: true, timeoutExpiresAt: duration ?
 ...
 */
async function generateReport(streamId, startDate, endDate) {
    try {
        ...
        const actionsQuery = await db.collection('streamModerationActions')
            ...
            .where('timestamp', '<=', end)
            .get();
        const actions = actionsQuery.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
        // Generate statistics
        const stats = {
 ...
 */
function getTopModerators(actions) {
 ...
 */
function getViolationTrends(actions) {
...
//# sourceMappingURL=moderationFunctions.js.map