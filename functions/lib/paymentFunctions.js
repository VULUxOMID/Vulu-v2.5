...
exports.generateFinancialReport = exports.processRevenueSharing = exports.handleStripeWebhook = exports.purchaseGems = void 0;
...
exports.purchaseGems = functions.https.onCall(async (data, context) => {
    try {
        ...
        if (!packageId || !paymentMethodId || !userId) {
            throw new functions.https.HttpsError('invalid-argument', 'packageId, paymentMethodId, and userId are required');
        }
        // Verify user matches authenticated user
        if (context.auth.uid !== userId) {
            throw new functions.https.HttpsError('permission-denied', 'User can only purchase gems for themselves');
        }
        ...
        const paymentIntent = await stripe.paymentIntents.create({
            amount: Math.round(gemPackage.price * 100),
            currency: 'usd',
        ...
        if (paymentIntent.status === 'succeeded') {
            ...
            const transactionId = await processGemPurchase(userId, packageId, gemPackage, paymentIntent.id);
            return {
                success: true,
                transactionId,
        ...
        throw new functions.https.HttpsError('internal', 'Failed to process gem purchase');
    }
});
/**
 ...
 */
exports.processRevenueSharing = functions.pubsub
 ...
 * Get gem package details
 */
function getGemPackage(packageId) {
    const packages = {
 ...
 */
async function processGemPurchase(userId, packageId, gemPackage, paymentIntentId) {
    try {
...
async function handlePaymentFailure(paymentIntent) {
    ...
    try {
        ...
        console.log(`Payment failed for user ${userId}: ${(_b = paymentIntent.last_payment_error) === null || _b === void 0 ? void 0 : _b.message}`);
    }
    catch (error) {
 ...
 */
async function generateTransactionReport(start, end) {
