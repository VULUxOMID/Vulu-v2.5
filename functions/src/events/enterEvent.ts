 ...
 */
export const enterEvent = functions.https.onCall(async (data, context) => {
  try {
    // Verify user is authenticated
    if (!context.auth) {
      throw new functions.https.HttpsError(
        'unauthenticated',
        'User must be authenticated to enter event'
      );
    }
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      if (currentGold < event.entryCost) {
        throw new functions.https.HttpsError(
          'failed-precondition',
          `Insufficient gold. You need ${event.entryCost} gold to enter.`
        );
      }
    ...
    throw new functions.https.HttpsError(
      'internal',
      `Failed to enter event: ${error.message}`
    );
  }
});
